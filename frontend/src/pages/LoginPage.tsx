  ...
  if (mfaToken) {
    return (
      <div className="bg-[#f7f9fe] min-h-screen flex flex-col items-center justify-center p-4">
        <MFAVerifyStep mfaToken={mfaToken} onCancel={() => setMfaToken(null)} />
      </div>
    )
  }
  ...
  return (
    <div className="bg-[#f7f9fe] text-[#181c20] min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden font-['Inter']">
      ...
      <div className="fixed top-0 left-0 w-full h-full -z-10 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] right-[-5%] w-[40%] h-[40%] bg-[#004ac6]/5 rounded-full blur-[120px]"></div>
        <div className="absolute bottom-[-10%] left-[-5%] w-[30%] h-[30%] bg-[#495c95]/5 rounded-full blur-[100px]"></div>
      </div>
      ...
      <main className="w-full max-w-[440px] bg-white p-10 rounded-2xl shadow-[0_12px_32px_-4px_rgba(0,74,198,0.08)] animate-in zoom-in-95 duration-500">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-[#434655] ml-1" htmlFor="email">Correo Electrónico</label>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center px-1">
              <label className="block text-sm font-semibold text-[#434655]" htmlFor="password">Contraseña</label>
              <a className="text-xs font-bold text-[#004ac6] hover:text-[#2563eb] transition-colors" href="#">¿Olvidó su contraseña?</a>
            </div>
            <div className="relative">
        ...
        <div className="mt-8 pt-8 border-t border-slate-50 text-center">
          <p className="text-sm text-[#434655]">
            ¿Aún no tiene acceso? 
            <Link className="text-[#004ac6] font-bold hover:underline ml-1" to="/register">Crear cuenta</Link>
          </p>
        </div>
      </main>

      {/* Footer */}
      <footer className="w-full py-8 mt-auto flex flex-col md:flex-row justify-between items-center px-8 space-y-4 md:space-y-0 text-[10px] font-medium uppercase tracking-widest text-[#434655]/50 max-w-7xl">
        <div className="flex items-center gap-4">
          <span>© 2024 Motor de Cumplimiento. The Sentinel’s Clarity Architecture.</span>
        </div>
        <div className="flex gap-8">
          <a className="hover:text-[#004ac6] transition-colors" href="#">Privacy Policy</a>
          <a className="hover:text-[#004ac6] transition-colors" href="#">Security Standards</a>
          <a className="hover:text-[#004ac6] transition-colors" href="#">Support</a>
        </div>
      </footer>
    </div>
  )
